          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              ...
              <el-upload
                ...
                style="width: 150px; margin-left: 10px"
              >
                <el-button size="small" type="info" icon="el-icon-upload2" @disabled="scope.row.updateAndReload"
                v-loading="scope.row.updateAndReload"
                element-loading-text="jar包正在更换中"
                element-loading-spinner="el-icon-loading"
                element-loading-background="rgba(0, 0, 0, 0.8)"
...
export default {
  ...
  methods: {
        ...
        .then((res) => {
          for (let v in res.data.data) {
            if (Object.prototype.hasOwnProperty.call(res.data.data, v)) {
              for (let i = 0; i < _this.tableData.length; i++) {
                if (_this.tableData[i].version === v) {
                  _this.$set(_this.tableData[i], "checkRes", res.data.data[v].startUser);
    ...
    shutdown(row) {
      ...
      this.$axios
        .post("http://192.168.0.187:5000/productJar/shutdown", {
          version: row.version,
    ...
    startup(row) {
      ...
      this.$axios
        .post("http://192.168.0.187:5000/productJar/startup", {
          version: row.version,
          user: this.$store.state.userInfo,
        })
        .then((res) => {
    ...
    reload(row) {
      ...
      this.$axios
        .post("http://192.168.0.187:5000/productJar/reload_product", {
          version: row.version,
          user: this.$store.state.userInfo,
        })
        .then((res) => {
    ...
    exchangeBihome(row, key) {
      this.$axios
        .post("http://192.168.0.187:5000/productJar/changeBihome", {
          version: row.version,
          bihome: key,
          user: this.$store.state.userInfo,
        })
